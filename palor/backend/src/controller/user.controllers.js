...
// Admin creates a walk-in user with minimal info
const createWalkInUser = asyncHandler(async (req, res) => {
    ...
    // Create new user with minimal info and a 'walk-in' role
    const user = await User.create({
        name,
        phone,
        role: "walk-in"
    ...
    res.status(201).json({
        success: true,
        message: "Walk-in user created successfully",
        data: user
    });
});
